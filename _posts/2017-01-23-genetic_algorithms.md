...
However, this difficulty can be partially overcomed by increasing the population size. A similar problem is tackled in the [Evolving Darwin](https://www.youtube.com/watch?v=dO05XcXLxGs) example, where a picture of Darwin is approximated with 1000 ellipses instead of 150 circles. The final product is almost identical to the original picture. However, the evolution took approximately the same number of generations as the Mona Lisa problem, and each step probably took longer in comparison due to larger population size. Thus, increasing the population makes the the approximation more accurate, but more time consuming.

Coming from a life science background, I really find this algorithm brilliant and intriguing. I hope you have enjoyed this as much as I did. Some of other fun examples that employed genetic algorithms as well as neural network are listed below: 
